...
def debug_db_status():
    ...
    try:
        ...
        gold_response = supabase.table('gold_prices').select('*').execute()
        strategy_response = supabase.table('investment_strategies').select('*').execute()
        
        return jsonify({
            "db_connection": "OK",
            "platform": "Vercel Serverless",
            "table_counts": {
                "gold_prices": len(gold_response.data) if gold_response.data else 0,
                "investment_strategies": len(strategy_response.data) if strategy_response.data else 0
            },
            "latest_updates": {
                "gold_prices": gold_response.data[-1]['created_at'] if gold_response.data else None,
                "investment_strategies": strategy_response.data[-1]['created_at'] if strategy_response.data else None
            },
            "sample_data": {
                "gold": gold_response.data[-1] if gold_response.data else None,
                "strategy": strategy_response.data[-1] if strategy_response.data else None
            }
        })
